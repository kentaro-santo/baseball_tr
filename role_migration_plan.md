...
playerListEl.addEventListener('change', async (e) => {
    ...
    if (!confirm(`このユーザーのロールを「${label}」に変更しますか？`)) {
        await renderPlayerList(); // キャンセル時は元に戻す
        return;
    }
...
💡 `fbGetCurrentUserRole()` は Firestore への読み取りが1回発生する。ログイン頻度は低いので問題ないが、将来的にキャッシュする場合は `localStorage` の活用を検討。
